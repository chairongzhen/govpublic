...
export default class BarChartRace extends Component {
        ...
        {!this.props.hideProgress && (
          <Fragment>
            ...
            <input
              id="input"
              className="progress"
              style={{ width: 'calc(100% - 400px)', marginBottom: 15 }}
              type="range"
  ...
  drawChart(data, config, width, height) {
    ...
    function redraw() {
      ...
      if (config.use_img) {
        barEnter
          ...
          .attr('width', '40')
          .attr('height', '40')
          .attr('href', d => config.images[d[nameField]]);
    ...
    const btn = iconG
      .append('image')
      .attr('href', 'assets/images/Path.svg')
      .attr('height', 20)
      .attr('width', 15)
      .attr('x', 30)
      .attr('y', 12)
    ...
    var inter = setInterval(function next() {}, baseTime * interval_time);

    function click(index, stop = false) {
      if (index !== undefined) i = index;
      if (clickParam % 2 === 0 && stop === false) {
        btn.attr('href', 'assets/images/pause1.svg');
        if (i >= time.length) {
          i = 0;
        ...
        inter = setInterval(function next() {
          ...
          if (i >= time.length) {
            clickParam++;
            btn.attr('href', 'assets/images/Path.svg');
        ...
        btn.attr('href', 'assets/images/Path.svg');
        window.clearInterval(inter);
        if (clickParam % 2 === 1 || stop) clickParam++;
      }
    }
    this.click = click;
  }
