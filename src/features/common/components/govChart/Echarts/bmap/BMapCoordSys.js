...
BMapCoordSys.prototype.dataToPoint = function (data) {
  var point = new BMapGL.Point(data[0], data[1]); // TODO mercator projection is toooooooo slow
...
BMapCoordSys.prototype.pointToData = function (pt) {
  ...
  var ptNew = this._bmap.overlayPixelToPoint({
    x: pt[0] + mapOffset[0],
    y: pt[1] + mapOffset[1]
  });

  return [ptNew.lng, ptNew.lat];
};
...
BMapCoordSys.create = function (ecModel, api) {
  ...
  ecModel.eachComponent('bmap', function (bmapModel) {
    ...
    if (!bmapModel.__bmap) {
      ...
      var bmap = bmapModel.__bmap = new BMapGL.Map(bmapRoot, { enableMapClick: false });
    ...
    if (center && zoom) {
      var pt = new BMapGL.Point(center[0], center[1]);
