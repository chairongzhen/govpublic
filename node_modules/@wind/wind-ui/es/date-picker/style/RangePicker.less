...
.@{calendar-prefix-cls}-range {
  ...
  .@{calendar-prefix-cls}-in-range-cell {
    ...
    &:before {
      content: '';
      display: block;
      background: @date-picker-range-cell-bg;
      border-radius: 0;
      border: 0;
      position: absolute;
      top: 1px;
  ...
  &.@{calendar-prefix-cls}-time {
    .@{calendar-timepicker-prefix-cls} {
      ...
      width: 100%;
      top: 60px;
      z-index: 2; // cover .w-calendar-range .w-calendar-in-range-cell > div (z-index: 1)
      &-panel {
        // height: 241px;
        margin-top: -@input-box-height;
      }

      &-inner {
        padding-top: @input-box-height;
        height: 213px;//100%
        background: none;
      }
